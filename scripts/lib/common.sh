...
# `scripts/`, where each has to be runnable from a fresh clone with nothing
# installed. These are cloned together, run from the same directory, and already
# depend on `compose.yaml` sitting beside them.
#
...
# time and the Server's own collector is anchored to UTC, so "what time did the
# backup actually run" is answerable from the log rather than from a guess about
# what the machine's zone was in March.
...
# The value of one variable, or a default, where a script wants a setting
...
# workstation does not. Calling it unconditionally reports a missing command as
# *another script is running*, which is a lie in the one situation where somebody
# is already confused.
#
...
lock() {
    ...
    # running*, naming its own parent's pid.
    #
...
# Removes images this project has stopped using, and nothing else.
prune_our_images() {
...
# to start in. A second copy of this arithmetic, or a hard-coded `$ROOT/backups`
# beside it, agrees only for the shipped default: an installation that moved its
# backups records an empty `dump_before`, and `rollback.sh` then offers a
# recovery command with no argument on it.
backup_dir() {
    local dir
...
# over its own root, PostgreSQL's `pg_isready`.
wait_healthy() {
    local timeout=${1:-120} waited=0
    while [ "$waited" -lt "$timeout" ]; do
        # **`-a`, and a delimiter that cannot appear in a value.** Without both,
        # this reports a crash-looping container as healthy:
        #
        ...
        #
        # Both Runners declare no health check, so the second is not theoretical:
        # `up --wait` reports them healthy while one is failing to sign in.
...
# Runs one psql command against the stack's own database, printing only the
# value.
psql_scalar() {
